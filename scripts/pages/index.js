...
recipes.forEach((recipe) => {
    ...
    recipeDiv.classList.add('recipe-card'); // Ajoutez une classe CSS pour personnaliser le style
...
<p class="prep-time">${recipe.time}min</p>
        ...
        ${recipe.ingredients.map((ingredient) => `
            <li>
                <span class="ingredient-type">${ingredient.ingredient}</span>
                <span class="ingredient-quantity">${ingredient.quantity !== undefined ? ingredient.quantity + ' ' + (ingredient.unit || '') : '-'}</span>
            </li>
        `).join('')}
    </ul>
    </div>
...
function searchRecipesInput() {
    ...
    updateRecipeDisplay(matchingRecipes);
}

function updateRecipeDisplay(matchingRecipes) {
    const recipesContainer = document.getElementById('recipe-container');
...
const defaultOption = document.getElementById('ingredients-combo');
const optionsContainer = document.getElementById('ingredients-options');

uniqueIngredients.forEach(optionText => {
...
document.addEventListener('click', event => {
  if (!document.getElementById('ingredients-combo').contains(event.target)) {
    optionsContainer.style.display = 'none';
    console.log('test3');
  }
});
...
const selectedItemsDisplay = document.getElementById("selected-items");

const ingredientSelect = document.getElementsByClassName("option");
...
// COMBOBOX USTENSILS
const ustensilNames = recipes.flatMap(recipe => recipe.ustensils);
const uniqueUstensils = [...new Set(ustensilNames)];

console.log(uniqueUstensils);

const defaultUstensilOption = document.getElementById('ustensils-combo');
const ustensilOptionsContainer = document.getElementById('ustensils-options');

uniqueUstensils.forEach(optionText => {
  const optionDiv = document.createElement('div');
  optionDiv.textContent = optionText;
  optionDiv.classList.add('option');
  optionDiv.addEventListener('click', () => {
    defaultUstensilOption.textContent = optionText;
    ustensilOptionsContainer.style.display = 'none';
    selectItem(optionText);
  });
  ustensilOptionsContainer.appendChild(optionDiv);
});

defaultUstensilOption.addEventListener('click', () => {
  if (ustensilOptionsContainer.style.display === 'block') {
    ustensilOptionsContainer.style.display = 'none';
  } else {
    ustensilOptionsContainer.style.display = 'block';
  }
});

// Fonction pour gérer la sélection d'ingrédients ou ustensiles
...
// COMBOBOX APPLIANCES
const applianceNames = recipes.flatMap(recipe => recipe.appliance);
const uniqueAppliance = [...new Set(applianceNames)];

console.log(uniqueAppliance);

const defaultApplianceOption = document.getElementById('appliances-combo');
const applianceOptionsContainer = document.getElementById('appliances-options');

uniqueAppliance.forEach(optionText => {
  const optionDiv = document.createElement('div');
  optionDiv.textContent = optionText;
  optionDiv.classList.add('option');
  optionDiv.addEventListener('click', () => {
  ...
  applianceOptionsContainer.appendChild(optionDiv);
});

defaultApplianceOption.addEventListener('click', () => {
    console.log('test23');
  if (applianceOptionsContainer.style.display === 'block') {
    applianceOptionsContainer.style.display = 'none';
  } else {
    applianceOptionsContainer.style.display = 'block';
  }
});
...
function displayPill() {
    selectedItemsDisplay.innerHTML = ''; // Effacez le contenu précédent
    const displayPill = document.createElement('div');
    displayPill.classList.add('displayPill');
  
    selectedItems.forEach((item) => {
      const pillContainer = document.createElement('div');
      pillContainer.classList.add('pillContainer');
      ...
      pillContent.classList.add('pillContent');
      ...
      // Ajoutez chaque pillContainer à displayPill
      displayPill.appendChild(pillContainer);
    });
  
    // Affichez ou masquez la section des "pills" en fonction de la longueur de selectedItems
    console.log(selectedItems);
    if (!selectedItems || selectedItems.length === 0) {
      selectedItemsDisplay.style.display = 'none'; // Masquer l'élément
    } else {
      selectedItemsDisplay.style.display = 'block'; // Afficher l'élément
    }
  
    // Ajoutez displayPill à selectedItemsDisplay
    selectedItemsDisplay.appendChild(displayPill);
  }

// La fonction pour fermer un "pill"
function closePill(item) {
    const itemIndex = selectedItems.indexOf(item);
    if (itemIndex !== -1) {
      selectedItems.splice(itemIndex, 1);
      displayPill();
  
      // Sélectionnez l'élément HTML par son ID
      const selectedItemsDisplay = document.getElementById("selected-items");
  
      // Masquez ou affichez l'élément en fonction de la longueur de selectedItems
...
// Sélectionnez tous les éléments d'option dans la liste d'ingrédients
const optionElements = document.querySelectorAll("#ingredients-options .option");

// Créez un tableau pour stocker les ingrédients sélectionnés
...
function toggleIngredientSelection(event) {
    ...
    // Vérifiez si l'option est déjà dans la liste des ingrédients sélectionnés
    if (selectedIngredients.includes(clickedOption.textContent)) {
        // Si c'est le cas, retirez-le de la liste
        const index = selectedIngredients.indexOf(clickedOption.textContent);
        selectedIngredients.splice(index, 1);
        clickedOption.classList.remove("selected");
    } else {
        // Sinon, ajoutez-le à la liste
        selectedIngredients.push(clickedOption.textContent);
        clickedOption.classList.add("selected");
    }

    // Vous pouvez faire ce que vous voulez avec la liste d'ingrédients sélectionnés ici
    console.log("Ingrédients sélectionnés : ", selectedIngredients);
}

// Ajoutez un gestionnaire d'événements "click" à chaque option
for (const option of optionElements) {
    option.addEventListener("click", toggleIngredientSelection);
}
  console.log(selectedItems)
  function searchRecipes() {
    const matchingRecipes = recipes.filter((recipe) => {
      ...
      return selectedItems.some((selectedItem) =>
        recipeName.includes(selectedItem.toLowerCase()) || recipeDescription.includes(selectedItem.toLowerCase())
      );
    });
        ...
        <div class=recipe-content>
          ...
          <ul class="ingredients-list">
            ${recipe.ingredients.map((ingredient) => `
              <li>
                <span class="ingredient-type">${ingredient.ingredient}</span>
                <span class="ingredient-quantity">${ingredient.quantity !== undefined ? ingredient.quantity + ' ' + (ingredient.unit || '') : '-'}
                </span>
              </li>
            `).join('')}
          </ul>
